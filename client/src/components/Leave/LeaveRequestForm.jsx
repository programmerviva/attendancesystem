      ...
      <div className="flex justify-between items-start mb-6">
        ...
        <div className="bg-blue-50 p-3 rounded-lg">
          <h3 className="text-sm font-medium text-blue-800 mb-2">
            Leave Balance (FY {financialYear.start.substring(0, 4)}-
            {financialYear.end.substring(0, 4)})
          </h3>
          <div className="grid grid-cols-2 gap-x-4 gap-y-1">
            <div className="text-xs">
              Sick Leave: <span className="font-medium">{leaveBalance.sick} days</span>
            </div>
            <div className="text-xs">
              Comp Off: <span className="font-medium">{leaveBalance.comp} days</span>
            </div>
            <div className="text-xs">
              Short Leave: <span className="font-medium">{leaveBalance.short} days</span>
            </div>
            <div className="text-xs">
              Vacation: <span className="font-medium">{leaveBalance.vacation} days</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="relative">
            ...
            >
              <span className="flex items-center justify-between">
                <span>
                  {leaveTypes.find((type) => type.id === leaveType)?.name || 'Select Leave Type'}
                </span>
                <svg
        ...
        <div className="border border-gray-200 rounded-lg p-4">
          <h3 className="text-sm font-medium text-gray-700 mb-2">
            {leaveType === 'comp'
              ? 'Select Comp Off Date to Use (Required)'
              : 'Select Comp Off Date to Use (Optional)'}
          </h3>
              ...
              <div className="flex">
                ...
                <div className="ml-3">
                  <h3 className="text-sm font-medium text-yellow-800">
                    No Comp Off Dates Available
                  </h3>
                  <div className="mt-2 text-sm text-yellow-700">
                    <p>
                      You don't have any comp off dates available. To earn comp off days, you need
                      to work on weekends or holidays.
                    </p>
                    {leaveType === 'comp' && (
                      <p className="mt-1 font-medium">
                        You cannot submit a Comp Off leave request without available comp off days.
                      </p>
                    )}
                  </div>
                </div>
        ...
        <div>
          <button
            ...
            disabled={
              loading ||
              (leaveType === 'comp' && (!selectedCompOffDate || compOffDates.length === 0))
            }
