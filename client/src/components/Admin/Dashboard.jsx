...
function Dashboard() {
  ...
  const fetchActivityData = async () => {
    try {
      ...
      const activityRes = await axios.get('http://localhost:5000/api/v1/activity/recent', {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const leavesRes = await axios.get('http://localhost:5000/api/v1/leaves/pending', {
        headers: { Authorization: `Bearer ${token}` }
      });
      
      // Fetch today's attendance
      const today = dayjs().format('YYYY-MM-DD');
      const attendanceRes = await axios.get(`http://localhost:5000/api/v1/attendance/summary?date=${today}`, {
        headers: { Authorization: `Bearer ${token}` }
                  ...
                  <div key={leave._id} className="border border-gray-200 rounded-lg p-4 hover:bg-gray-50">
                    <div className="flex justify-between items-start">
                      <div>
                        <p className="text-sm font-medium text-gray-900">
                          {leave.user?.fullName?.first || 'Employee'} {leave.user?.fullName?.last || ''}
                        </p>
                        <p className="text-xs text-gray-500">{leave.user?.department || 'Department'}</p>
                      </div>
                    ...
                    <div className="mt-2">
                      <p className="text-sm text-gray-600">
                        <span className="font-medium">{leave.leaveType.charAt(0).toUpperCase() + leave.leaveType.slice(1)} Leave:</span> {formatDate(leave.startDate)} to {formatDate(leave.endDate)}
                      </p>
