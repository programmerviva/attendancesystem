      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        <div className="bg-gradient-to-r from-blue-600 to-blue-800 px-6 py-4 flex justify-between items-center">
          <h2 className="text-xl font-bold text-white">
            {employeeInfo ? `${employeeInfo.fullName.first} ${employeeInfo.fullName.last}'s Attendance` : 'Employee Attendance'}
          </h2>
          <button 
            onClick={onClose}
            ...
            <div className="mb-6 bg-gray-50 p-4 rounded-lg">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <p className="font-medium">{employeeInfo.fullName.first} {employeeInfo.fullName.last}</p>
                </div>
                <div>
          ...
          <div className="mt-6 bg-gray-50 p-4 rounded-lg">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div>
                ...
                <p className="font-medium">{attendanceData.filter(record => 
                  record.checkIn?.time && dayjs(record.checkIn.time).hour() < 10 || 
                  (dayjs(record.checkIn.time).hour() === 10 && dayjs(record.checkIn.time).minute() < 30)
                ).length}</p>
              </div>
              <div>
                ...
                <p className="font-medium">{attendanceData.filter(record => 
                  record.checkIn?.time && 
                  ((dayjs(record.checkIn.time).hour() === 10 && dayjs(record.checkIn.time).minute() >= 30) || 
                   (dayjs(record.checkIn.time).hour() > 10 && dayjs(record.checkIn.time).hour() < 13))
                ).length}</p>
              </div>
              <div>
                <p className="text-sm text-gray-500">Half Days</p>
                <p className="font-medium">{attendanceData.filter(record => 
                  record.checkIn?.time && dayjs(record.checkIn.time).hour() >= 13
                ).length}</p>
              </div>
              <div>
                <p className="text-sm text-gray-500">Absent Days</p>
                <p className="font-medium">{dayjs().year(currentYear).month(currentMonth).daysInMonth() - attendanceData.length}</p>
              </div>
            </div>
          </div>
        </div>
