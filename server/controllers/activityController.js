...
export const getRecentActivity = async (req, res, next) => {
  try {
    ...
    recentAttendance.forEach(record => {
      if (record.checkIn && record.checkIn.time) {
        attendanceActivities.push({
          ...
          user: record.user ? `${record.user.fullName.first} ${record.user.fullName.last}` : 'Unknown User',
          action: 'Checked in',
    ...
    const leaveActivities = recentLeaves.map(leave => ({
      id: `leave-${leave._id}`,
      user: leave.user ? `${leave.user.fullName.first} ${leave.user.fullName.last}` : 'Unknown User',
